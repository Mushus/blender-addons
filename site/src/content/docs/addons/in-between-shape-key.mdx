...
3. 各ターゲット行のスライダーをドラッグするか数値を入力して、キーの位置を `0.000`〜`1.000` の範囲で変更します。表示値は名前から直接読み取り、操作すると名前も即時に変わります（例: `Smile@0.5`）。`@.5` や `@0.500` は無効です。`@0` はValueが0のときに適用されます。最大位置のキー以降は100%まで同じ形状を維持します。
