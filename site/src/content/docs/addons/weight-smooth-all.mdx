...
```mermaid
flowchart LR
  sel[SelectedVerts] --> smooth[SmoothUnlockedDeformVGs]
  smooth --> norm[NormalizeSelected]
  locked[LockedOrNonBoneVG] -.->|no write| norm
  unsel[UnselectedVerts] -.->|no write| norm
```
