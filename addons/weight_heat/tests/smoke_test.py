...
def make_skinned_strip(name: str):
    """2 ボーン + 3 頂点の短冊。中央頂点だけ選択して拡散効果を見る。"""
...
def test_enable_vertex_select_then_heat():
    """背景: Weight Paint で頂点選択マスクが無いと選択が効かない。
    なぜ: 初回クリックでマスク有効化、2 回目で拡散する契約を固定する。
    """
    obj, arm = make_skinned_strip("Weight Heat")
    bpy.context.view_layer.objects.active = obj
    obj.select_set(True)
    bpy.ops.object.mode_set(mode="WEIGHT_PAINT")
    ...
    assert bpy.ops.weight_heat.heat() == {"FINISHED"}
    ...
    settings = bpy.context.scene.weight_heat
    ...
    # 熱拡散は純粋近傍平均: (1.0+0)/2=0.5 に収束
    assert abs(after_center["BoneA"] - 0.5) < 1e-5
    assert abs(after_center["BoneB"] - 0.5) < 1e-5
...
def test_locked_and_missing_armature():
    ...
    """
    obj, arm = make_skinned_strip("Weight Heat Lock")
...
def test_no_selection():
    ...
    """
    obj, arm = make_skinned_strip("Weight Heat Empty")
    enter_weight_paint_vertex_select(obj, set())
    assert bpy.ops.weight_heat.heat() == {"CANCELLED"}
    cleanup(obj, arm)


def main():
    options = parse_args()
    temp_dir = Path(tempfile.mkdtemp(prefix="blender-weight-heat-smoke-"))
    extract_zip(Path(options.zip).resolve(), temp_dir)
    sys.path.insert(0, str(temp_dir))

    addon = importlib.import_module("weight_heat")
    addon.register()
    host_state = bpy.app.driver_namespace["blender_addon_tools.embedded_host.v1"]
    assert "weight_heat" in host_state["tools"]
    assert "weight_utility" in host_state["groups"]
    clear_factory_mesh()
    try:
        test_enable_vertex_select_then_heat()
        ...
        if bpy.context.mode != "OBJECT":
            bpy.ops.object.mode_set(mode="OBJECT")
        addon.unregister()
    print("Heat Weight functional smoke test passed")
